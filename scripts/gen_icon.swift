...
for (name, px) in entries {
    ...
    NSGradient(
        starting: NSColor(red: 0.18, green: 0.65, blue: 0.45, alpha: 1),
        ending: NSColor(red: 0.12, green: 0.50, blue: 0.35, alpha: 1)
    )?.draw(in: bg, angle: -90)

    if let symbol = tintedSymbol("externaldrive.fill", size: s) {
